/**
 * PeerCard component - displays individual peer with actions
 */

import { useState } from "react";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent, CardHeader } from "@/components/ui/card";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  AlertDialogContent,
  AlertDialogDescription,
  AlertDialogFooter,
  AlertDialogHeader,
  AlertDialogTitle,
} from "@/components/ui/alert-dialog";
import type { PeerDto } from "@/types";
import { cn } from "@/lib/utils";

interface PeerCardProps {
  peer: PeerDto;
  onUpdate: (peer: PeerDto) => void;
  onDelete: (id: string) => void;
  onDownload: (id: string) => void;
}

export function PeerCard({ peer, onUpdate, onDelete, onDownload }: PeerCardProps) {
  const [showRevokeDialog, setShowRevokeDialog] = useState(false);
  const [isRevoking, setIsRevoking] = useState(false);

  const isRevoked = peer.revoked_at !== null;
  const isActive = peer.status === "active" && !isRevoked;

  const handleRevoke = async () => {
    setIsRevoking(true);
    try {
      await onDelete(peer.id);
      setShowRevokeDialog(false);
    } catch (error) {
      console.error("Failed to revoke peer:", error);
    } finally {
      setIsRevoking(false);
    }
  };

  const formatDate = (dateString: string | null) => {
    if (!dateString) return "N/A";
    return new Date(dateString).toLocaleDateString("en-US", {
      year: "numeric",
      month: "short",
      day: "numeric",
    });
  };

  return (
    <>
      <Card
        className={cn(
          "transition-shadow hover:shadow-md cursor-pointer",
          isRevoked && "opacity-60"
        )}
        onClick={() => onUpdate(peer)}
        role="article"
        aria-label={`Configuration: ${peer.friendly_name || "Unnamed"}`}
      >
        <CardHeader className="pb-3">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold truncate">
                {peer.friendly_name || "Unnamed Configuration"}
              </h3>
              <p className="text-xs text-muted-foreground font-mono truncate">
                {peer.public_key.substring(0, 32)}...
              </p>
            </div>
            <Badge
              variant={isActive ? "default" : "secondary"}
              className={cn(
                isActive && "bg-green-500 hover:bg-green-600",
                isRevoked && "bg-gray-400"
              )}
            >
              {isRevoked ? "Revoked" : peer.status}
            </Badge>
          </div>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-between">
            <div className="text-xs text-muted-foreground">
              <p>Claimed: {formatDate(peer.claimed_at)}</p>
              {isRevoked && (
                <p className="text-destructive">Revoked: {formatDate(peer.revoked_at)}</p>
              )}
            </div>
            <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
              {/* Download Button */}
              <button
                onClick={() => onDownload(peer.id)}
                disabled={isRevoked}
                className={cn(
                  "p-2 rounded-md transition-colors",
                  isRevoked
                    ? "text-gray-400 cursor-not-allowed"
                    : "text-primary hover:bg-accent"
                )}
                title="Download configuration"
                aria-label="Download configuration"
              >
                <svg
                  className="h-5 w-5"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth={2}
                  stroke="currentColor"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"
                  />
                </svg>
              </button>

              {/* Edit Button */}
              <button
                onClick={() => onUpdate(peer)}
                className="p-2 rounded-md text-primary hover:bg-accent transition-colors"
                title="Edit friendly name"
                aria-label="Edit friendly name"
              >
                <svg
                  className="h-5 w-5"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth={2}
                  stroke="currentColor"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"
                  />
                </svg>
              </button>

              {/* Revoke Button */}
              {!isRevoked && (
                <button
                  onClick={() => setShowRevokeDialog(true)}
                  className="p-2 rounded-md text-destructive hover:bg-destructive/10 transition-colors"
                  title="Revoke configuration"
                  aria-label="Revoke configuration"
                >
                  <svg
                    className="h-5 w-5"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth={2}
                    stroke="currentColor"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
                    />
                  </svg>
                </button>
              )}
            </div>
          </div>
        </CardContent>
      </Card>

      {/* Revoke Confirmation Dialog */}
      <AlertDialog open={showRevokeDialog} onOpenChange={setShowRevokeDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Revoke Configuration?</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to revoke this configuration? This action cannot be
              undone. The configuration will no longer be usable.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isRevoking}>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={handleRevoke}
              disabled={isRevoking}
              className="bg-destructive hover:bg-destructive/90"
            >
              {isRevoking ? "Revoking..." : "Revoke"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
  );
}

